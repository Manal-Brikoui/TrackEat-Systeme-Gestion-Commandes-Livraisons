...
import { UserPlus, ArrowRight, Eye, EyeOff, Check, X } from 'lucide-react'
...
export default function RegisterPage() {
  ...
  const submit = async (e) => {
    e.preventDefault()
    if (!allPassed) { toast.error('Le mot de passe ne respecte pas les critères'); return }
    setLoading(true)
    try {
      const res = await authService.register(form)
      const token = res.token
      if (!token) throw new Error('Token manquant dans la réponse')
      const user = { id: res.id, email: res.email, fullName: res.fullName, role: res.role, phone: res.phone }
      setAuth(user, token)
      toast.success('Compte créé avec succès !')
      nav(roleHome(res.role))
    } catch (err) {
      const msg = err.response?.data?.message || err.response?.data || err.message || "Erreur lors de l'inscription"
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .te-grid { display:grid; grid-template-columns:1fr 1fr; gap:18px; }
        @media (max-width:480px) { .te-grid { grid-template-columns:1fr; } }
      `}</style>

      <div style={{ minHeight:'100vh', display:'flex', background:'#fafaf7', fontFamily:"'Jost',sans-serif", position:'relative', overflow:'hidden' }}>
        <div style={{ position:'absolute', width:600, height:600, borderRadius:'50%', background:'rgba(183,234,78,.06)', filter:'blur(90px)', top:-150, left:-150, pointerEvents:'none' }} />
        <div style={{ position:'absolute', width:450, height:450, borderRadius:'50%', background:'rgba(99,102,241,.05)', filter:'blur(80px)', bottom:-100, right:-100, pointerEvents:'none' }} />
        ...
        <div style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'48px 20px', position:'relative', zIndex:2 }}>
          <div style={{ width:'100%', maxWidth:540, animation:'slideUp .4s cubic-bezier(.22,1,.36,1) both' }}>
            ...
            <div style={{ background:'#fff', border:'1px solid rgba(23,23,20,.08)', borderRadius:28, padding:'42px 40px', boxShadow:'0 12px 56px rgba(23,23,20,.07)' }}>
              <form onSubmit={submit} style={{ display:'flex', flexDirection:'column', gap:20 }}>
                ...
                <div className="te-grid">
                  <div>
                    <label className="te-label">Nom complet</label>
                    <input
                      className="te-input"
                ...
                <button className="te-btn" disabled={loading || !allPassed}>
                  {loading ? <div className="te-spinner" /> : <><UserPlus size={16} />Créer mon compte</>}
                </button>
              </form>
              ...
              <div style={{ textAlign:'center', color:'rgba(23,23,20,.5)', fontSize:'.9rem' }}>
                Déjà un compte ?{' '}
                <Link to="/login" style={{ color:'#171714', fontWeight:700, textDecoration:'none', display:'inline-flex', alignItems:'center', gap:4 }}>
